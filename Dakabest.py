...
def load_data():
    global REPLY_TEXT1, REPLY_TEXT2, LAST_SCAN, SESSIONS, DELAY_RANGE, SCANNING_DAYS
...
def is_service_message(message):
    """⭐ Check if message is a service message (join/leave/etc)"""
    if message.service:
        return True
    # Additional checks for service-type messages
    if message.text and any(keyword in message.text.lower() for keyword in ["joined", "left", "was removed"]):
        return True
    return False
...
async def run_parallel_attacks(app_list, chat_id, msg_id, text, times, task_id, cancel_event=None):
...
def register_handlers(app, all_apps=None):
    ...
    async def show_current(client, message):
        ...
        text += "To cancel a task, use: `/cancel <task_id>`\nExample: `/cancel 123456`"
    ...
    async def devour_menu(client, message):
        ...
        if state["step"] == "main_menu":
            ...
            # 1) Scan all users (with date filter and service message filter)
            if opt == "1":
                state["step"] = "scanning"
        ...
        if state["step"] == "attack_link_confirm":
            ...
            if all_apps:
                task = asyncio.create_task(
                    run_parallel_attacks(all_apps, chat_id, msg_id, text_to_send, times, task_id, cancel_event=state["cancel_event"])
                )
    ...
    async def cancel_task(client, message):
        ...
        if len(parts) == 1:
            ...
            for task_id, task_data in user_tasks.items():
                text += f"Task ID: `{task_id}` | Mode: `{task_data['mode']}` | Progress: `{task_data['count']}/{task_data['total']}`\n"
